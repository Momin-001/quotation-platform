...
/**
 * One LEDitor display workspace: product picker, wall preview, derived summary,
 * installation/controller options and the enquiry form.
 *
 * Rendered once per comparison tab. All three instances stay mounted so a
 * configured display survives tab switching, which is why `isActive` is threaded
 * through to the canvas and the reCAPTCHA widget.
 */
export default function LeditorWorkspace({
    displayIndex = 0,
    isActive = true,
    preselectSlug = null,
    onSnapshotChange,
    ...
    // All three displays are mounted together, so ids and radio-group names have to
    // be scoped per display or the browser treats them as one control.
    const uid = useCallback((name) => `${name}-${displayIndex}`, [displayIndex]);
    ...
    // Latch on first activation so an unvisited tab costs nothing, and so returning
    // to a tab does not refetch the catalogue it already has.
    const [catalogEnabled, setCatalogEnabled] = useState(isActive);
    useEffect(() => {
        if (isActive) setCatalogEnabled(true);
    }, [isActive]);

    useEffect(() => {
        if (!catalogEnabled) return;
        fetchProducts();
    }, [catalogEnabled, fetchProducts]);
    ...
    useEffect(() => {
        ...
        if (!canvas || !imagesLoaded || !isActive) return;

        const ctx = canvas.getContext("2d");
        // Zero while the panel is hidden or the accordion is collapsed; drawing then
        // would compute negative dimensions and blow up the canvas allocation.
        ...
        // `isActive` matters: a hidden tab reports zero parent width, so the wall has
        // to be redrawn once this panel is actually visible.
    }, [config, computed.cabInfo, imagesLoaded, userPreviewImageLoaded, isActive]);
    ...
    // Report this display's state upward for the comparison dialog. `computed` is
    // already memoised, so this only fires when the numbers actually change.
    // Memoised so the shell's identity check can bail out; a fresh object every run
    // would re-render all three workspaces on every keystroke.
    const snapshot = useMemo(
        () => (selectedProduct ? { product: selectedProduct, config, computed } : null),
        [selectedProduct, config, computed]
    );

    useEffect(() => {
        onSnapshotChange?.(displayIndex, snapshot);
    }, [displayIndex, onSnapshotChange, snapshot]);

    // The widget unmounts when this tab is hidden, so drop the token with it.
    // Otherwise the form still holds a stale value while the box reads unchecked.
    useEffect(() => {
        if (!isActive) setValue("captcha", "", { shouldValidate: false });
    }, [isActive, setValue]);

    // Apply a configuration copied from another display.
    const appliedCopyRef = useRef(null);
    useEffect(() => {
        if (!copyPayload || copyPayload.token === appliedCopyRef.current) return;
        appliedCopyRef.current = copyPayload.token;
        if (!copyPayload.product) return;
        setSelectedProduct(copyPayload.product);
        setConfig({ ...copyPayload.config });
    }, [copyPayload]);
    ...
    return (
        <div className="space-y-4">
                ...
                >
                    <div id={uid("screen-info-section")} className="space-y-6">
                        {!selectedProduct ? (
                ...
                >
                    <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full">
                            <div className="space-y-1.5">
                                <Label htmlFor={uid("ipRating")} className="text-sm font-medium">
                                    {t("ipRating")}
                                </Label>
                                <Input
                                    id={uid("ipRating")}
                ...
                <LeditorSectionAccordion
                    ...
                    className="scroll-mt-24"
                >
                    <div id={uid("submit-enquiry-section")}>
                        {isAuthenticated ? (
                            <form
                            ...
                            >
                                ...
                                <div className="space-y-1.5">
                                    <Label htmlFor={uid("enquiry-project-name")} className="text-sm font-medium">
                                        {t("projectName")}
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <Label htmlFor={uid("enquiry-message")} className="text-sm font-medium">
                                            {t("yourMessage")}
                                            <span className="text-destructive ml-0.5">*</span>
                                        </Label>
                                        <Textarea
                                            id={uid("enquiry-message")}
                                            {...register("message")}
                                            placeholder={
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pt-1">
                                    <div className="min-w-0">
                                        {/* One live widget at a time: three mounted
                                            reCAPTCHAs interfere with each other. */}
                                        {isActive ? (
                                            <ReCAPTCHA
