...
export function getEnquiryDisplayTitle(
    ...
    return first?.product?.productName || first?.productName || productEnquiry;
}
...
export function getQuotationDefaultUnitPrice(product, { isOptionalItem = false, isAdditionalItem = false } = {}) {
    ...
    if (isOptionalItem || product.sourceType === "accessory") {
        return product.retailPrice != null && product.retailPrice !== "" ? String(product.retailPrice) : "";
    }
    if (isAdditionalItem || product.sourceType === "controller") {
        return product.pricePerControllerUsd != null && product.pricePerControllerUsd !== ""
            ? String(product.pricePerControllerUsd)
            : "";
    }
    return product.pricePerCabinetUsd != null && product.pricePerCabinetUsd !== ""
        ? String(product.pricePerCabinetUsd)
        : "";
}
