...
/** Display title for an enquiry (project name, otherwise main product name). */
export function getEnquiryDisplayTitle(
    enquiry,
    { productEnquiry = "Product enquiry" } = {}
) {
    const projectName = enquiry?.projectName?.trim();
    if (projectName) return projectName;

    const first = enquiry?.items?.[0];
    return (
        first?.product?.productName ||
        first?.refurbishedProduct?.serie ||
        first?.productName ||
        productEnquiry
    );
}
...
/**
 * Sell price for a catalog item: base price uplifted by its profit margin factor.
 * A margin of 0.5 on a base of 1000 yields 1000 + 1000 * 0.5 = 1500.
 * A missing or unparseable margin leaves the base price untouched.
 */
export function applyProfitMargin(basePrice, profitMargin) {
    if (basePrice == null || basePrice === "") return "";
    const base = parseFloat(basePrice);
    if (isNaN(base)) return "";

    const margin = parseFloat(profitMargin);
    if (isNaN(margin)) return String(base);

    return (base + base * margin).toFixed(2);
}

/** Default unit price from catalog: products, controllers, or accessories. */
export function getQuotationDefaultUnitPrice(product, { isOptionalItem = false, isAdditionalItem = false } = {}) {
    if (!product) return "";
    if (isOptionalItem || product.sourceType === "accessory") {
        return applyProfitMargin(product.purchasePrice, product.profitMargin);
    }
    if (isAdditionalItem || product.sourceType === "controller") {
        return applyProfitMargin(product.pricePerControllerUsd, product.profitMargin);
    }
    return applyProfitMargin(product.pricePerCabinetUsd, product.profitMargin);
}
